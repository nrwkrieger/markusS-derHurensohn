--[[
    ...
    GitHub: https://github.com/nrwkrieger/markusS-derHurensohn
]]
...
local instanceId = {}
safeWriteGlobal("LurkInstanceId", instanceId)

local LurkUI = {}
LurkUI.__index = LurkUI
...
local Theme = {
	...
	AccentSecondary = Color3.fromRGB(0, 180, 220),
...
LurkUI.ToggleKey = 0xA1
LurkUI.ToggleKeyName = "Right-Shift"
LurkUI.Visible = true
LurkUI.LastToggleState = false
...
function LurkUI:Notify(title, text, duration)
...
function LurkUI:CreateWindow(config)
	local window = {
	...
	LurkUI:Notify(window.title, "Press " .. LurkUI.ToggleKeyName .. " to toggle UI", 5)
	...
	task.spawn(function()
		...
		while true do
			local menuTogglePressed = iskeypressed(LurkUI.ToggleKey)
			if menuTogglePressed and not LurkUI.LastToggleState then
				LurkUI.Visible = not LurkUI.Visible
			end
			LurkUI.LastToggleState = menuTogglePressed

			for _, obj in ipairs(window.globalDrawingObjects) do
				obj.Visible = LurkUI.Visible
			end
			for _, tabData in ipairs(window.tabs) do
				tabData.btn.Visible = LurkUI.Visible
			end
			if not LurkUI.Visible then
